  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        <View style={styles.infoCard}>
          ...
          {caseItem.nextHearing && (
            <InfoRow
              ...
              value={formatDate(caseItem.nextHearing)}
              highlight
              last
            />
          )}
        ...
        <Text style={styles.sectionTitle}>Sobre o Processo</Text>
        <View style={styles.descCard}>
          <Text style={styles.description}>{caseItem.description}</Text>
        </View>

        <Text style={styles.sectionTitle}>Atualizações ({caseItem.updates.length})</Text>
        {caseItem.updates.map((u, idx) => (
          <View key={u.id} style={styles.updateCard}>
            <View style={styles.timeline}>
              <View style={[styles.dot, idx === caseItem.updates.length - 1 && { backgroundColor: colors.secondary }]} />
              {idx < caseItem.updates.length - 1 && <View style={styles.line} />}
            </View>
        ...
        {caseDocs.length > 0 && (
          <>
            <Text style={styles.sectionTitle}>Documentos do Processo</Text>
            {caseDocs.map((d) => (
              <View key={d.id} style={styles.docCard}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={styles.docMeta}>{d.category} · {formatDate(d.uploadedAt)}</Text>
                </View>
...
function InfoRow({ icon, label, value, highlight, last }: {
  icon: string; label: string; value: string; highlight?: boolean; last?: boolean;
}) {
  return (
    <View style={[styles.infoRow, !last && styles.infoRowBorder]}>
      <Ionicons name={icon as any} size={16} color={highlight ? colors.warning : colors.primary} style={{ marginRight: 8 }} />
